...
function check5OfKind(input) {
    ...
    return temp.removecount === 5 ? 1 : null;
}

function check4OfKind(input) {
    input = input.split("")
    let temp = arrayRemove(input, input[0])
    if (temp.removecount === 4) return 2
    else {
        temp = arrayRemove(temp.arr, temp.arr[0])
    }
    return temp.removecount === 4 ? 2 : null;
}
...
function compare(item1, item2) {
    let compare_array = ['A', 'K', 'Q', 'J', 'T', '9', '8', '7', '6', '5', '4', '3', '2'];
